...
function renderSideBar(items) {
	...
	if (sideBarContainer) {
		...
		items.forEach(item => {
			// Создаем родительский элемент side-bar-item
			const sideBarItem = document.createElement('div');
			sideBarItem.classList.add('side-bar-item');

			// Добавляем родительский заголовок с иконкой стрелки
			sideBarItem.innerHTML = `
            <div class="side-bar-item__content">
                <span>${item.catalogItemTitle}</span>
                <img src="img/icons/arrow-right-a.svg" alt="arrow-right">
            </div>
            <div class="side-bar-item__subcontent">
                <div class="subcontent-elems">
                    ${item.catalogItemElements.map(element => `
                        <div class="side-bar-item">
                            <div class="side-bar-item__content">
                                <span>${element.catalogItemElementName}</span>
                            </div>
                        </div>
                    `).join('')}
                </div>
