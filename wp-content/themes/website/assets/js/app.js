...
async function catalogLogic() {
	...
	catalogTriggers.forEach((catalogTrigger) => {
		catalogTrigger.addEventListener('mouseenter', (e) => {
			renderCatalog(catalogItems).then(() => {
				moveTitles(); // Перемещаем заголовки после рендеринга
...
function moveTitles() {
	...
	const footerItems = document.querySelectorAll('.footer-top-item');

	if (window.innerWidth > 860) {
		document.querySelector('.catalog-mobile-trigger').removeAttribute('data-spoller');
		footerItems.forEach((footerItem) => {
			footerItem.removeAttribute('data-spollers');
			footerItem.querySelector('.footer-top-item__title').removeAttribute('data-spoller');
		})
	} else {
		document.querySelector('.catalog-mobile-trigger').setAttribute('data-spoller', '');
		footerItems.forEach((footerItem) => {
			footerItem.setAttribute('data-spollers', '');
			footerItem.querySelector('.footer-top-item__title').setAttribute('data-spoller', '');
		})
	}
        ...
        <div class="product-item">
            <div class="product-slider swiper">
				<div class="swiper-wrapper">
					${product.images.map(image => `<div class="swiper-slide"><img src=" <?php echo get_template_directory_uri() ?>  /assets/${image}" alt="${product.name}"></div>`).join("")}
				</div>
